...
public class JasonUtilAction {
    ...
    private void getContacts(final JSONObject action, final JSONObject data, final JSONObject event, final Context context) {
        ...
        try {
            if (ActivityCompat.checkSelfPermission((Activity)context, Manifest.permission.READ_CONTACTS) != PackageManager.PERMISSION_GRANTED) {
    ...
    public void openWhatsapp(final JSONObject action, JSONObject data, final JSONObject event, final Context context) {
        String whatsappNumber = null;
        String whatsappMsg = null;
        String url = null;
        try {
            if (action.has("options")) {
                JSONObject options = action.getJSONObject("options");
                if(options.has("whatsappNumber") || options.has("whatsappMsg")){
                    whatsappNumber = options.getString("whatsappNumber");
                    whatsappMsg = URLEncoder.encode(options.getString("whatsappMsg"), "UTF-8");

                    String regEx = "^((\\+92)?(0092)?(92)?(0)?)(3)([0-9]{9})$";
                    CharSequence inputStr = whatsappNumber;
                    Pattern pattern = Pattern.compile(regEx, Pattern.UNICODE_CASE);
                    Matcher matcher = pattern.matcher(inputStr);

                    if(!whatsappMsg.equals("") && whatsappNumber.equals("")){
                        url = "https://api.whatsapp.com/send?text=" + whatsappMsg;
                    }else {
                        if(matcher.matches()) {
                            url = "https://api.whatsapp.com/send?phone=" + whatsappNumber + "&text=" + whatsappMsg;
                        } else {
                            url = "https://chat.whatsapp.com/" + whatsappNumber;
                        }
                    }
                    ...
                    err.put("message", "Please invalid data string");
                    JasonHelper.next("error", action, err, event, context);
                }
            }
            else {
                JSONObject err = new JSONObject();
                err.put("message", "$util.openWhatsapp has no options defined");
                JasonHelper.next("error", action, err, event, context);
            }
        } catch (Exception e) {
            Log.d("Warning", e.getStackTrace()[0].getMethodName() + " : " + e.toString());
